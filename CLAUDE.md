...
| `none`     | `migrate-claude.yml`, `migrate-infer.yml` (infra/tooling repos with no sync/agent family; intentionally **not** swept by `stale.yml` / `cleanup-runs.yml`) |
...
- `.kind != "none"` - `stale.yml`, `cleanup-runs.yml` (every product repo except the `kind: none` infra repos).
